...
static int  write_chroma_intra_pred_mode (Macroblock* currMB,int rdopt);
static int  write_chroma_coeff           (Macroblock* currMB);
static int  write_CBP_and_Dquant         (Macroblock* currMB);

///////////////////////////////////////////////////////////////////////////////
//! single scan pattern                                  //先列后行。。。
const byte SNGL_SCAN1[16][2] =
{
	{ 0, 0 }, { 1, 0 }, { 0, 1 }, { 0, 2 },
	{ 1, 1 }, { 2, 0 }, { 3, 0 }, { 2, 1 },
	{ 1, 2 }, { 0, 3 }, { 1, 3 }, { 2, 2 },
...
extern int cn;
const short map_macroblock_show[16] =
{ 0, 1, 4, 5, 2, 3, 6, 7, 8, 9, 12, 13, 10, 11
, 14, 15 };
const short map_macroblock_encryption_1[9] =
{ 3, 6, 7, 9, 12, 13, 11, 14, 15
};
const short map_macroblock_encryption_2[12] =
{ 2, 3, 6, 7, 8, 9, 12, 13, 10, 11, 14, 15
};
const short map_macroblock_encryption_3[12] =
{ 1, 4, 5, 3, 6, 7, 9, 12, 13, 11, 14, 15
};
///////////////////////////////////////////////////////////////////////
...
short logistic_generate(short mod)
{
	...
	return ((__int64)(temp*1.0e+14)) % mod;
}
////////////////////////////////////////////////////////////////////////////////////////////////

static char value_encryption(char value, int mod)
{
	//return value^logistic_generate(mod);
	//////////////////////////////////////////////
	cn = value^logistic_generate(mod); //value值异或logistic映射值
	return cn;
	//////////////////////////////////////////////
}

//////////////////////////////////////////////////////////////////////////////////////////////
static void show_matrix_value(int * m)
{
	int i;
	putchar('\n');
	for (i = 0; i<16; i++)
	{
		printf("%d ", m[map_macroblock_show[i]]);
		if ((i + 1) % 4 == 0)
			putchar('\n');
	}
}

void encryption_intra_mode(char * m, int location_of_macroblock) //m为该16*16宏块预测帧内预测模式数组
{
	static short count = 0;
	int i;
	//if(count++<1)
	if (location_of_macroblock == 0) //能正常预测帧内预测的16*16宏块
	{
		for (i = 0; i<16; i++)
		{
			if (m[map_macroblock_show[i]] != -1) //如果该宏块预测值不为-1
				m[map_macroblock_show[i]] = value_encryption(m[map_macroblock_show[i]], 8); //将该宏块预测值置乱
			else if (en_ps.intra_mode == 2) //如果为-1且帧内加密模式为2
			{
				m[map_macroblock_show[i]] = 1;
				m[map_macroblock_show[i]] = value_encryption(m[map_macroblock_show[i]], 8);
			}
		}
	}
	else if (location_of_macroblock == 1) //最左上角16*16宏块
	{
		for (i = 0; i<9; i++)
			if (m[map_macroblock_encryption_1[i]] != -1) //如果该宏块预测值不为-1
				m[map_macroblock_encryption_1[i]] = value_encryption(m[map_macroblock_encryption_1[i]], 8);
			else if (en_ps.intra_mode == 2) //如果为-1且帧内加密模式为2
			{
				m[map_macroblock_show[i]] = 1;
				m[map_macroblock_show[i]] = value_encryption(m[map_macroblock_show[i]], 8);
	...
	else if (location_of_macroblock == 2) //第一行16*16宏块
	{
		for (i = 0; i<12; i++)
			if (m[map_macroblock_encryption_2[i]] != -1) //如果该宏块预测值不为-1
				m[map_macroblock_encryption_2[i]] = value_encryption(m[map_macroblock_encryption_2[i]], 8);
			else if (en_ps.intra_mode == 2) //如果为-1且帧内加密模式为2
			{
				m[map_macroblock_show[i]] = 1;
				m[map_macroblock_show[i]] = value_encryption(m[map_macroblock_show[i]], 8);
	...
	else if (location_of_macroblock == 3) //第一列16*16宏块
	{
		for (i = 0; i<12; i++)
			if (m[map_macroblock_encryption_3[i]] != -1) //如果该宏块预测值不为-1
				m[map_macroblock_encryption_3[i]] = value_encryption(m[map_macroblock_encryption_3[i]], 8);
			else if (en_ps.intra_mode == 2) //如果为-1且帧内加密模式为2
			{
				m[map_macroblock_show[i]] = 1;
				m[map_macroblock_show[i]] = value_encryption(m[map_macroblock_show[i]], 8);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
int MVD_encryption(int mvd, int cavlc)
{
	int absmvd, i, j, sub = 0, temp = 1, codenum;
	absmvd = abs(mvd); //求当前MVD的绝对值
	codenum = (mvd>0) ? 2 * absmvd - 1 : 2 * absmvd; //映射成码字
	for (i = 0; i<16 && absmvd != 0; i++)
	{
		absmvd /= 2; //等同于i=floor[log2(Codenum+1)]
	}
	if (i) //i>0
	{
		for (j = 0; j <= i - 1; j++)
		{
			sub += temp; //Sub=2^i-1 码字结构的下限
			temp *= 2; //temp最终等于该组码字结构取值范围的长度
		}
		if (cavlc == 2)
			sub += temp;
		if (i != 0)
		{
			if (cavlc == 2)
				codenum = value_encryption(codenum, sub);
			else
			{
...
static void DCT_no_coeff_encryption(int * pLevel, int * Run, int cdc, int DCT)
{
				...
				if (DCT>2)
				{
					if (numPrefix <= 2 && vlcnum <= 5)
					{
						if (p == lastcoeff - numtrailingones&&level_two_or_higher == 1)
							numPrefix = (numPrefix - 1 + logistic_generate(2)) % 2 + 1;
						// numPrefix=value_encryption(numPrefix-1,2)+1;
						else
							numPrefix = (numPrefix + logistic_generate(3)) % 3;
						// numPrefix=value_encryption(numPrefix,3);
					}
					if (DCT>3)
					{
						if (numPrefix>2 && vlcnum <= 5)
						{
							numPrefix = (numPrefix - 3 + logistic_generate(12)) % 12 + 3;
							// numPrefix=value_encryption(numPrefix-3,12)+3;
						}
						if (vlcnum == 6)
						{
...
static void DCT_encryption(int DCT, Macroblock *currMB)
{
	Slice *currSlice = currMB->p_Slice;
	//VideoParameters *p_Vid = currMB->p_Vid;
	int MB_non_coef[4][4][4][4] = { 0 };
	//Macroblock* currMB = &currSlice->mb_data[p_Vid->current_mb_nr];
	int b8, b4;
	...
	if (currMB->mb_type==I16MB)//I16MB
		DCT_no_coeff_encryption(currSlice->cofDC[0][0], currSlice->cofDC[0][1], 0, DCT);
	for (b8 = 0; b8<4; b8++)
		for (b4 = 0; b4<4; b4++)
			DCT_no_coeff_encryption(currSlice->cofAC[b8][b4][0], currSlice->cofAC[b8][b4][1], 0, DCT);
	if (currMB->cbp>15)
		for (uv = 0; uv<2; uv++)
			DCT_no_coeff_encryption(currSlice->cofDC[uv + 1][0], currSlice->cofDC[uv + 1][1], 1, DCT);
	if (currMB->cbp>31)
	{
		for (b8 = 4; b8<6; b8++)
			for (b4 = 0; b4<4; b4++)
...
************************************************************************
*/
static int writeIntra4x4Modes(Macroblock *currMB,int rdopt)
{
	Slice *currSlice = currMB->p_Slice;
	///////////////////////////////////////////////
	VideoParameters *p_Vid = currMB->p_Vid;
	...
	if (en_ps.intra_mode)
	{
		if (rdopt == 0 && currMB->mb_type == I4MB)
		{
			if (mb_nr == 0) //左上角
				location_of_macroblock = 1;
			else if (mb_nr<p_Vid->PicWidthInMbs) //第一行
				location_of_macroblock = 2;
			else if (mb_nr%p_Vid->PicWidthInMbs == 0) //第一列
				location_of_macroblock = 3;
			if (en_ps.intra_mode == 1)
				encryption_intra_mode(currMB->intra_pred_modes, location_of_macroblock);
			else
				encryption_intra_mode(currMB->intra_pred_modes, location_of_macroblock);
		}
	}
...
static int writeIntraModes(Macroblock *currMB,int rdopt)
{
	switch (currMB->mb_type)
	{
	case I4MB:
		return writeIntra4x4Modes(currMB,rdopt);
		break;
	case I8MB:
...
static int writeIPCMLuma (Macroblock *currMB, imgpel **imgY, Bitstream *currStream, SyntaxElement *se, int *bitCount)
{
  ...
  *bitCount += bit_size;
  ...
  {      
    ...
    {            
      se->bitpattern = imax(p_Vid->min_IPCM_value*100, imgY[j][i]*100);
...
static int writeIPCMChroma (Macroblock *currMB, imgpel **imgUV, Bitstream *currStream, SyntaxElement *se, int *bitCount)
{
  ...
  *bitCount += bit_size;
  ...
  for (j = currMB->pix_c_y; j < currMB->pix_c_y + p_Vid->mb_cr_size_y; ++j)
  {
    ...
    {
      se->bitpattern = imax(p_Vid->min_IPCM_value*100, imgUV[j][i]*100);
      se->value1 = se->bitpattern;
#if TRACE
...
int write_b_slice_MB_layer (Macroblock *currMB, int rdopt, int *coeff_rate)
{
  ...
  if (currMB->mb_type == P8x8)
  {
    ...
    no_bits += currSlice->writeMotionInfo2NAL (currMB,rdopt);
  }
  ...
  no_bits += writeIntraModes(currMB,rdopt);
  ...
  if (currMB->IntraChromaPredModeFlag && ((p_Vid->active_sps->chroma_format_idc != YUV400) && (p_Vid->active_sps->chroma_format_idc != YUV444)))
    no_bits += write_chroma_intra_pred_mode(currMB,rdopt);
 ...
 ************************************************************************
 */
int write_chroma_intra_pred_mode(Macroblock* currMB, int rdopt)
{
  Slice* currSlice = currMB->p_Slice;
  ///////////////////////////////////////////////
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  if (en_ps.chroma_mode)
  {
	  if (mb_nr == 0)
		  location_of_macroblock = 1;
	  else if (mb_nr<p_Vid->PicWidthInMbs)
		  location_of_macroblock = 2;
	  else if (mb_nr%p_Vid->PicWidthInMbs == 0)
		  location_of_macroblock = 3;
	  if (rdopt == 0 &&
		  location_of_macroblock == 0)
		  currMB->c_ipred_mode = value_encryption(currMB->c_ipred_mode, 4);
  }
  ///////////////////////////////////////////////////////////////////////
...
void write_macroblock (Macroblock* currMB, int eos_bit)
{
  ...
  /////////////////////////////////////////////////////
  if (en_ps.DCT)
	  DCT_encryption(en_ps.DCT, currMB);
  /////////////////////////////////////////////////////////
                          ...
                          )
{
  ...
  int            rate       = 0;
  int            step_h     = part_size[mv_mode][0]; //水平方向分的块数
  int            step_v     = part_size[mv_mode][1]; //垂直方向分的块数
  SyntaxElement  se;
  BitCounter      *mbBits   = &currMB->bits;
    ...
    for (i = i0; i < i1; i += step_h)
    {
      ...
      for (k=0; k<2; ++k)
      {
		...
		if (en_ps.MVD == 1)
		{
			if (rdopt == 0)
			{
				int b8,b4,i2,j2,p,num=0;
				int block4x4cofAdd[4][4][1]={0};
				int MBcofAdd = 0, AVEblock4x4cofAdd = 0;
				int keep = 1;
				for (b8 = 0; b8<4; b8++)
				for (b4 = 0; b4<4; b4++)
				{
					for (p = 0; currSlice->cofAC[b8][b4][0][p] != '\0'; p++)
						block4x4cofAdd[b8][b4][0] += currSlice->cofAC[b8][b4][0][p];
					MBcofAdd += block4x4cofAdd[b8][b4][0];
					num++;
				}
				AVEblock4x4cofAdd = MBcofAdd / num; //AC系数的平均值
				for (j2 = j; j2<j + step_v&&keep; j2++)
				for (i2 = i; i2<i + step_h&&keep; i2++)
				if (block4x4cofAdd[2 * (j2 / 2) + i2 / 2][2 * (j2 % 2) + i2 % 2][0]>AVEblock4x4cofAdd)
				{
					curr_mvd = MVD_encryption(curr_mvd, en_ps.MVD + 1);
					keep = 0;
				}
				if (keep)
					curr_mvd = MVD_encryption(curr_mvd, en_ps.MVD);
			}
		}
		if (en_ps.MVD == 2)
		{
 ...
 ************************************************************************
 */
int write_b_slice_motion_info_to_NAL (Macroblock* currMB,int rdopt)
{
  int   no_bits         = 0;
      ...
      for (i0=0; i0<4; i0+=step_h0)
      {
        ...
        if (currMB->b8x8[k].mode !=0 && (currMB->b8x8[k].pdir == 0 || currMB->b8x8[k].pdir == 2))//has forward vector
        {
          ...
          no_bits  += writeMotionVector8x8 (currMB, i0, j0, i0 + step_h0, j0 + step_v0, motion[currMB->block_y + j0][currMB->block_x + i0].ref_idx[LIST_0], 
            LIST_0, currMB->b8x8[k].mode, currMB->b8x8[k].bipred,rdopt);
        }
      }
    }
      ...
      for (i0 = 0; i0 < 4; i0 += step_h0)
      {
        ...
        {          
          no_bits  += writeMotionVector8x8 (currMB, i0, j0, i0+step_h0, j0+step_v0, motion[currMB->block_y + j0][currMB->block_x + i0].ref_idx[LIST_1], 
            LIST_1, currMB->b8x8[k].mode, currMB->b8x8[k].bipred,rdopt);
        }
      }
    }
  }
 ...
 ************************************************************************
 */
int write_p_slice_motion_info_to_NAL (Macroblock* currMB,int rdopt)
{
  int   no_bits         = 0;
  //=== If multiple ref. frames, write reference frame for the MB ===
  if (currMB->mb_type != I4MB  && currMB->mb_type != I16MB && currMB->mb_type != I8MB  && currMB->mb_type != 0)
