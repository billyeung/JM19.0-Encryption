...
typedef struct pix_pos //像素坐标 与8.6无差别
{
  int   available;
  int   mb_addr;
  short x; ////像素在所属宏块中的坐标,以像素为单位或子宏块为单位
  short y;
  short pos_x; //像素在宏块中的绝对坐标
  short pos_y;
} PixelPos; //!!!!!
...
{
  struct slice       *p_Slice;                    //!< pointer to the current slice 当前条带指针
  struct video_par   *p_Vid;                      //!< pointer to VideoParameters 参数
  ...
  struct macroblock_dec   *mbup;   // neighbors for loopfilter 给滤波器的?不重要
  ...
  int           i16mode;			//亮度Intra16*16预测
  char          b8mode[4];			//本宏块每个8*8分块的最佳预测模式 8.6里是int现在是char了
  char          b8pdir[4];
  char          ipmode_DPCM;
  char          c_ipred_mode;       //!< chroma intra prediction mode 色度Intra预测
  char          skip_flag;
  short         DFDisableIdc;
...
typedef struct slice
{
  ...
  unsigned int current_mb_nr; // bitstream order 8.6里有 当前编码的宏块号
  unsigned int num_dec_mb;
  short        current_slice_nr; ////一幅图像内当前编码的条带号 计数功能??
  //int mb_x;
  //int mb_y;
  //int block_x;
  //int block_y;
  ...
  int erc_mvperMB;
  Macroblock *mb_data;				//!<一帧图像当前的?宏块数据，以宏块地址寻访
  struct storable_picture *dec_picture;
  int **siblock;
  byte **ipredmode;
  char  *intra_block;				//标识图像中的各个宏块是否为帧内预测块(可用于帧内预测)
...
typedef struct coding_par
{
  ...
  BlockPos *PicPos;  
  byte **ipredmode;                  //!< prediction type [90][74] //预测模式
...
typedef struct video_par
{
  ...
  int newframe;
  int structure;                     //!< Identify picture structure type 帧or场
